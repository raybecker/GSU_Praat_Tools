...
# rescaleSound is part of GSU Praat Tools 1.9. GSU Praat Tools is
# free software; you can redistribute it and/or modify it under the
# terms of the GNU General Public License as published by the Free
# Software Foundation; either version 3 of the License, or (at your
# option) any later version.

# rescaleSound is distributed in the hope that it will be useful,
...
form rescale Sound
  ...
  comment Rescaling strategy...
  optionmenu Rescaling_method 1  
    option set rescaling method first
    option multiply by scalar
    option multiply by window
    option scale peak amplitude
    option scale intensity (dB)
  positive Multiply_by_scalar 1.0
  optionmenu Multiply_by_window 1
    option set window shape
    option rectangular
    option triangular
    option parabolic
    option Hanning
    ...
    option Kaiser1
    option Kaiser2
  positive Scale_peak_amplitude 0.99
  positive Scale_intensity_dB 70.0
...
if ('rescaling_method' = 3)
  if ('multiply_by_window' <> 1)
    if ('multiply_by_window' = 2)
      windowShape$ = "rectangular"
     elsif ('multiply_by_window' = 3)
      windowShape$ = "triangular"
     elsif ('multiply_by_window' = 4)
      windowShape$ = "parabolic"
     elsif ('multiply_by_window' = 5)
      windowShape$ = "Hanning"
     elsif ('multiply_by_window' = 6)
      windowShape$ = "Hamming"
     elsif ('multiply_by_window' = 7)
      windowShape$ = "Gaussian1"
     elsif ('multiply_by_window' = 8)
      windowShape$ = "Gaussian2"
     elsif ('multiply_by_window' = 9)
      windowShape$ = "Gaussian3"
     elsif ('multiply_by_window' = 10)
      windowShape$ = "Gaussian4"
     elsif ('multiply_by_window' = 11)
      windowShape$ = "Gaussian5"
     elsif ('multiply_by_window' = 12)
      windowShape$ = "Kaiser2"
     elsif ('multiply_by_window' = 13)
      windowShape$ = "Kaiser2"
    endif
   else
    # error and exit
    clearinfo
    printline
    printline Error: select window type
    printline
    exit
  endif
endif
...
procedure editor_nolabels
  ...
  # call the rescale routine: rescale
  call rescale
...
procedure rescale
  ...
  if ('rescaling_method' <> 1)
    if ('rescaling_method' = 2)
      Multiply... 'multiply_by_scalar'
     elsif ('rescaling_method' = 3)
      if ('multiply_by_window' <> 1)
        Multiply by window... 'windowShape$'
      endif
    elsif ('rescaling_method' = 4) 
     Scale peak... scale_peak_amplitude
   else 
    Scale intensity... scale_intensity_dB
  endif
endproc
