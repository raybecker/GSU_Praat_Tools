...
# stationaryAverage is part of GSU Praat Tools 1.9. GSU Praat Tools is
# free software; you can redistribute it and/or modify it under the
# terms of the GNU General Public License as published by the Free
# Software Foundation; either version 3 of the License, or (at your
# option) any later version.

# stationaryAverage is distributed in the hope that it will be useful,
...
############################################################
procedure average_the_waveform
  Down to Matrix
  matrixNumber = selected ("Matrix", 1)
  matrixName$ = selected$ ("Matrix", 1)
  numberOfColumns = Get number of columns
  i = 1
  while 'i' <= ('numberOfColumns' - 'averaging_points')
   runningTotal = 0
    for j from 'i' to ('i' + 'averaging_points' - 1)
      currentValue = Get value in cell... 1 'j'
      runningTotal = 'runningTotal' + 'currentValue'
    endfor
    newValue = ('runningTotal' / 'averaging_points')
    for j from 'i' to ('i' + 'averaging_points' - 1)
       Set value... 1 'j' 'newValue'
    endfor
    i = ('i' + 'averaging_points')
  endwhile
  To Sound (slice)... 1
  modfiedTargetSegmentNumber = selected ("Sound", 1)
  modfiedTargetSegmentName$ = selected$ ("Sound", 1)
endproc
...
procedure clean_up
  select 'targetsegmentNumber'
   plus 'matrixNumber'
   plus 'modfiedTargetSegmentNumber'
  Remove
endproc
