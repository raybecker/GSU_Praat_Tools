...
form add Noise
  ...
  comment Noise parameters...
  optionmenu Noise_type 1
    option gaussian noise
    option uniform noise
  positive Relative_amplitude 0.1
...
############################################################
procedure add_noise
  Copy... TargetSegmentModified
  targetsegmentModifiedNumber = selected ("Sound")
  targetsegmentModifiedName$ = selected$ ("Sound")
  # make and scale the noise file (mean = 0.0, stdev = 0.1)
  if noise_type = 1
    Create Sound... TempNoise 0 'durTarget' 'sf' randomGauss(0,0.1)
   elsif noise_type = 2
    Create Sound... TempNoise 0 'durTarget' 'sf' randomUniform(0,0.1)
  endif
  # get the number of the new file, scale
  noisesoundNumber = selected ("Sound")
  select 'noisesoundNumber'
  Scale... (0.99 * 'relative_amplitude')
  # add this noise to the target segment
  select 'targetsegmentModifiedNumber'
  Formula... self + Sound_TempNoise(x)
endproc
