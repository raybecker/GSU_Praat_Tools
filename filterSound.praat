...
# filterSound is part of GSU Praat Tools 1.9. GSU Praat Tools is
# free software; you can redistribute it and/or modify it under the
# terms of the GNU General Public License as published by the Free
# Software Foundation; either version 3 of the License, or (at your
# option) any later version.

# filterSound is distributed in the hope that it will be useful,
...
form filter Sound
  ...
  optionmenu Filtering_method 1
    option pass Hann band
    option stop Hann band
    option smooth the waveform
  positive From_frequency_(Hz) 500
  positive To_frequency_(s) 1000
  optionmenu Smoothing_points 1
    option 3
    option 5
    option 7
    option 9
...
# set remaining variables for filtering
smoothing = 100
...
procedure editor_nolabels
  ...
  if ('filtering_method' < 3)
    call filtering_band
   else
    call filtering_smoothing
  endif

  # if the entire file was done, set first point equal to second point,
  # routine may have missed changing the first point
  if beginTarget = 'sp'
    pointTwo = Get value at index... 2
    Set value at index... 1 pointTwo
  endif

  # paste the modified segment back into original file, clean up
  call paste_back_in
  call clean_up
...
procedure editor_withlabels
  ...
  select 'targetsegmentModifiedNumber'
  # paste the modified segment back into original file, clean up
  call paste_back_in
  call clean_up
...
procedure objects_nolabels
  ...
  for k from 1 to 'numberOfSelectedSounds'
    ...
    # open in sound editor, paste in, cut out original
    call paste_back_in
    call clean_up
...
############################################################
procedure filtering_band
  Filter ('filtering_method$')... 'from_frequency' 'to_frequency' 'smoothing'
  Multiply... 'multiply_by_scalar'
  Rename... TargetSegmentModified
  targetsegmentModifiedNumber = selected ("Sound")
  targetsegmentModifiedName$ = selected$ ("Sound")
endproc

############################################################
procedure filtering_smoothing
  # make a matrix from the segment
  Down to Matrix
  matrix1Number = selected ("Matrix")
  # make a matching empty matrix
  numberOfSamples = Get number of columns
  Create simple Matrix... Smoothed 1 'numberOfSamples' 0
  matrix2Number = selected ("Matrix")
  # combine the matrices
  select 'matrix1Number'
  plus 'matrix2Number'
  Merge (append rows)
  matrix3Number = selected ("Matrix")
  # compute smoothed values in row 2 from values in row 1 
  call smooth_the_waveform
  # convert these values to a Sound file, set sample rate to original
  To Sound (slice)... 2
...
############################################################
procedure smooth_the_waveform
  if ('smoothing_points' = 1) 
    Formula... ((self[1,col])+(self[1,col-1])+(self[1,col+1])) / 3
   elsif ('smoothing_points' = 2) 
          Formula... ((self[1,col])+(self[1,col-2])+(self[1,col-1])+(self[1,col+1])+(self[1,col+2])) / 5
   elsif ('smoothing_points' = 3) 
    Formula... ((self[1,col])+(self[1,col-3])+(self[1,col-2])+(self[1,col-1])+(self[1,col+1])+
      ...(self[1,col+2])+(self[1,col+3])) / 7
   elsif ('smoothing_points' = 4)
    Formula... ((self[1,col])+(self[1,col-4])+(self[1,col-3])+(self[1,col-2])+(self[1,col-1])+
      ...(self[1,col+1])+(self[1,col+2])+(self[1,col+3])+(self[1,col+4])) / 9
   elsif ('smoothing_points' = 5)
    Formula... ((self[1,col])+(self[1,col-5])+(self[1,col-4])+(self[1,col-3])+(self[1,col-2])+
      ...(self[1,col-1])+(self[1,col+1])+(self[1,col+2])+(self[1,col+3])+(self[1,col+4])+(self[1,col+5])) / 11
   elsif ('smoothing_points' = 6)
   Formula... ((self[1,col])+(self[1,col-6])+(self[1,col-5])+(self[1,col-4])+(self[1,col-3])+
      ...(self[1,col-2])+(self[1,col-1])+(self[1,col+1])+(self[1,col+2])+(self[1,col+3])+
      ...(self[1,col+4])+(self[1,col+5])+(self[1,col+5])) / 13
...
procedure paste_back_in
  ...
  Edit
  editor Sound 'targetsegmentModifiedName$'
  Select... 0.0 'durTarget'
...
procedure clean_up
  select 'targetsegmentNumber'
    plus 'targetsegmentModifiedNumber'
  if 'filtering_method' = 3
    plus 'matrix1Number'
    plus 'matrix2Number'
    plus 'matrix3Number'
