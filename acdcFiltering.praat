...
procedure editor_nolabels
  ...
  call check_selection_window_mismatch_ed
...
procedure objects_nolabels
  ...
  # alert user and quit if selection and window size/placement are mismatched
  call check_selection_window_mismatch_ob
...
procedure place_window_nolabels
...
############################################################
 # set beginTarget and endTarget according to window placement/size
procedure place_window_withlabels
  # set beginTarget and endTarget according to window placement/size
  if 'window_placement' = 1
    # use entire interval
...
############################################################
procedure adjust_boundaries_to_zero
...
############################################################
procedure check_selection_window_mismatch_ed
...
############################################################
procedure check_selection_window_mismatch_ob
...
############################################################
procedure extract_target_segment
  Extract sound selection (time from 0)
  Close
  endeditor
  targetsegmentNumber = selected ("Sound")
...
procedure main_program
  ...
  if ('remove_AC' > 1)
    ...
    oldTargetSegmentDuration = 'durTarget'
    call paste_back_in
  endif
  # normalize amplitude
