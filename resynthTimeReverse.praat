...
# resynthTimeReverse is part of GSU Praat Tools 1.9. GSU Praat Tools is
# free software; you can redistribute it and/or modify it under the
# terms of the GNU General Public License as published by the Free
# Software Foundation; either version 3 of the License, or (at your
# option) any later version.

# resynthTimeReverse is distributed in the hope that it will be useful,
...
form time-reverse waveforms
  ...
  optionmenu Segment_length_(ms) 1
    option 10
    option 25
    option 50
    option 75
    ...
    option 200
    option all
  real Custom_segment_length_(ms) 0
...
if ('segment_length' = 1)
  segmentLength = 10
 elsif ('segment_length' = 2)
  segmentLength = 25
 elsif ('segment_length' = 3)
  segmentLength = 50
 elsif ('segment_length' = 4)
  segmentLength = 75
 elsif ('segment_length' = 5)
  segmentLength = 100
 elsif ('segment_length' = 6)
  segmentLength = 125
 elsif ('segment_length' = 7)
  segmentLength = 150
 elsif ('segment_length' = 8)
  segmentLength = 175
 elsif ('segment_length' = 9)
  segmentLength = 200
 elsif ('segment_length' = 10)
  segmentLength = 10000000
endif

# segment length adjustment to sec
if ('custom_segment_length' > 0)
  segmentLength = 'custom_segment_length'
endif
...
procedure editor_nolabels
  ...
  # find number of TR segments
  if ('segmentLength' > 1000)
     numberOfSegments = 1
   else
     numberOfSegments = round ('durTarget' / 'segmentLength')
  endif
  durTRSegment = ('durTarget' / 'numberOfSegments')

  # select and reverse each segment in turn
  beginSegment = 'beginTarget'
  for x from 1 to 'numberOfSegments'
    endSegment = ('beginSegment' + 'durTRSegment')
    Select... 'beginSegment' ('beginSegment'+'durTRSegment')
    Reverse selection
    beginSegment = 'beginSegment' + 'durTRSegment'
  endfor
  endeditor
...
procedure editor_withlabels
  ...
  # extract segment to Objects window
  Extract selected sound (preserve times)
  Close
  endeditor
  segmentNumber = selected ("Sound",1)
  Rename... TargetSegment
  segmentName$ = "TargetSegment"
  Edit
  editor Sound 'segmentName$'
  ...
  # copy the entire segment to the clipboard
  Select... 0 10000
  Copy selection to Sound clipboard
  Close
  endeditor

  # paste back into orginal file
  select 'soundNumber'
  Edit
  editor Sound 'soundName$'
  Select... 'beginTarget' 'beginTarget'
  Paste after selection
  Select... 'endTarget' ('endTarget'+'durTarget')
  Cut
  endeditor
...
procedure save_data_to_disk
  select 'soundNumber'
  Write to WAV file... 'dataDirectoryPath$''sl$'tr'soundName$'.wav
endproc
